...
# -*- coding:utf-8 -*-
"""
File: /workspace/code/VideoPose3D/main.py
...
-----
Last Modified: Wednesday October 8th 2025 4:51:54 pm
...
config = hydra.compose(config_name="videopose3d.yaml")
...
person_pt_dataset = list(Path(config.pt_path).glob("**/*.pt"))
dataset = CustomDataset(person_pt_dataset[0])
...
def render():
    ...
    for sbj_test in subjects_test:
        ...
        prediction = evaluate(
            gen, return_predictions=True
        )  # * here predict the 3d pose
        ...
        if args.viz_output is not None:
            ...
            _opt_path = args.viz_output + "/{}_{}_{}.gif".format(
                sbj_test, args.viz_action, args.viz_camera
            )
            _ipt_video_path = (
                "/workspace/data/side_raw/"
                + sbj_test.split("_")[0]
                + "_"
                + sbj_test.split("_")[1]
                + "/"
