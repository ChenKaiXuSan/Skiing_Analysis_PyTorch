...
# -*- coding:utf-8 -*-
"""
File: /workspace/code/videopose3d/run.py
...
-----
Last Modified: Wednesday September 3rd 2025 3:32:39 pm
...
def run_video_pose_3d(config: DictConfig, pt_path: Path, out_dir: Path, args):
    ...
    args.viz_export = str(out_dir)

    os.makedirs(args.viz_output.rsplit("/", 1)[0], exist_ok=True)
    os.makedirs(args.viz_export.rsplit("/", 1)[0], exist_ok=True)

    print("Loading dataset...")

    # TODO: 这里加载一个人的一个视点的pt数据呢
    dataset = CustomDataset(pt_path=pt_path, remove_static_joints=True)
    ...
    keypoints = np.load(
        "/workspace/VideoPose3D/data/data_2d_custom_myvideos.npz", allow_pickle=True
    )
    # TODO: 这里需要换成pt文件的
    ...
    for k, v in dataset._data.items():
        print("Subject {}: {} actions".format(k, len(v.keys())))
        keypoints[k] = {"custom": [v["custom"]["detectron2"]["keypoints"].numpy()]}
    ...
    for subject in dataset.subjects():
        ...
        for action in dataset[subject].keys():
            ...
            for cam_idx in range(len(keypoints[subject][action])):
                ...
                mocap_length = dataset[subject][action]["positions_3d"][cam_idx].shape[
                    0
                ]
    ...
    if not args.render:
        ...
        subjects_test = args.viz_subject.split(",")
    ...
    if not args.evaluate:
        ...
        if semi_supervised:
            ...
            optimizer = optim.Adam(
                list(model_pos_train.parameters())
        ...
        while epoch < args.epochs:
                ...
                if not args.no_eval:
                    ...
                    if semi_supervised:
                        for cam, _, batch_2d in semi_generator_eval.next_epoch():
                            cam = torch.from_numpy(cam.astype("float32"))
                            inputs_2d_semi = torch.from_numpy(
                                batch_2d.astype("float32")
                            )
                            if torch.cuda.is_available():
                                cam = cam.cuda()
                                inputs_2d_semi = inputs_2d_semi.cuda()
            ...
            if args.export_training_curves and epoch > 3:
                ...
                if semi_supervised:
                    ...
                    plt.legend(
                        ["traj. train", "traj. train (eval)", "traj. valid (eval)"]
                    )
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            for _, batch, batch_2d in test_generator.next_epoch():
                ...
                epoch_loss_3d_pos += (
                    inputs_3d.shape[0] * inputs_3d.shape[1] * error.item()
                )
                N += inputs_3d.shape[0] * inputs_3d.shape[1]
    ...
    if args.render:
        ...
        for sbj_test in subjects_test:

            input_keypoints = keypoints[sbj_test][args.viz_action][
                args.viz_camera
            ].copy()
            ground_truth = None
            if sbj_test in dataset.subjects() and args.viz_action in dataset[sbj_test]:
                if "positions_3d" in dataset[sbj_test][args.viz_action]:
                    ground_truth = dataset[sbj_test][args.viz_action]["positions_3d"][
                        args.viz_camera
                    ].copy()
            ...
            if args.viz_export is not None:
                ...
                np.save(
                    os.path.join(args.viz_export, dataset.get_video_name() + ".npy"),
                    prediction,
                )
            ...
            if args.viz_output is not None:
                ...
                render_animation(
                    ...
                    size=args.viz_size,
                    input_video_path=_ipt_video_path,
