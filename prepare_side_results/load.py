...
def load_info(
    ...
    return (
        kpts_xy.astype(dtype, copy=False) if kpts_xy is not None else None,
        kpt_scores.astype(dtype, copy=False) if kpt_scores is not None else None,
        bboxes_xyxy.astype(dtype, copy=False) if bboxes_xyxy is not None else None,
        bbox_scores.astype(dtype, copy=False) if bbox_scores is not None else None,
        frames.numpy(),
    )
