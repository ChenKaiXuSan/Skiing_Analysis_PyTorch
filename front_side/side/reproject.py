...
# -*- coding:utf-8 -*-
'''
File: /workspace/code/front_side/side/reproject.py
Project: /workspace/code/front_side/side
...
-----
Last Modified: Thursday December 25th 2025 4:27:24 pm
...
def reproject_points(
    ...
    if R.ndim == 3:
        ...
        if R.shape[0] != 2 or T.shape[0] != 2:
            raise ValueError(
                f"Expect R,T shape (2,3,3),(2,3), got {R.shape}, {T.shape}"
            )
        ...
        if R.shape != (3, 3) or T.shape not in [(3,), (3, 1)]:
            raise ValueError(
                f"Expect R(3,3), T(3,) for relative extrinsic, got {R.shape}, {T.shape}"
            )
        R_rel = R
        t_rel = T.reshape(3, 1)
    else:
